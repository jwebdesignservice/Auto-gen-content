...
## Reference Screenshots — Jack Supplies Them

For case study campaigns (or anything requiring real product imagery), Jack provides the screenshots directly — do not attempt to fetch or screenshot client websites from this environment, the sandbox proxy blocks outbound HTTPS to arbitrary hosts.

Workflow:
1. Jack pastes/uploads PNGs into the conversation, or drops them into `campaigns/<slug>/references/`.
2. Read each PNG to extract the visible product copy and visual style.
3. Build the deck around them.
